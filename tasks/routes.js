...
const { createTask, getAllTasks, registerUser, loginUser, updateTasks,  deleteTasks} = require('./controller');  // 引入 getAllTasks 函數
const authenticateToken = require('../middleware/authMiddleware'); // 引入 JWT 驗證中介軟體

// 註冊與登入路由
router.post('/register', registerUser);  // 註冊
router.post('/login', loginUser);        // 登入
